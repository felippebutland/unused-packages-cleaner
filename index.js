...
export default async function removeUnusedDependencies({ action }) {
  ...
  const packageJsonModule = await import(packageJsonUrl, {
    assert: { type: 'json' },
  });
  ...
  if (action === 'remove') {
    await removeDependencies(
      formatDependencyList(packageJson.dependencies.unusedDeps),
      'Uninstalling unused dependencies...'
    );
    await removeDependencies(
      formatDependencyList(packageJson.devDependencies.unusedDeps),
      'Uninstalling unused devDependencies...'
    );
  } else if (action === 'analyze') {
    console.log(
      '\x1b[32m',
      'Unused dependencies:',
      packageJson.dependencies.unusedDeps
    );
    console.log(
      '\x1b[32m',
      'Unused devDependencies:',
      packageJson.devDependencies.unusedDeps
    );
  } else {
...
async function removeDependencies(dependencies, message) {
  if (dependencies) {
    console.log(`we'll remove those ${dependencies}`);
